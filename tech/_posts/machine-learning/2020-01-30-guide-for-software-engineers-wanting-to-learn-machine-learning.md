...
Data scientists are strong in mathematics, they are expert in playing with data and designing efficient models. These might not be exactly your forte, but you need to slowly develop these capabilities. Without that, a model will remain a black box. Now you have the map of the ML terrain, and can locate a problem and potential algorithms on that map. You are ready to go on the journey of improving the understanding of workings of ML models (with the help of courses and books listed earlier) and adapting engineer practices to ML.
